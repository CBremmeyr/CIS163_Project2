...
public class MineSweeperGame {
    ...
    public void select(int row, int col) throws
        ...
        if(selectedCell.getMineCount() == 0 && !(selectedCell.isExposed())) {
            selectedCell.setExposed(true);
    ...
    private void checkIfWon(){
        ...
        for (int i = 0; i < boardSize; i++) {
            for (int j = 0; j < boardSize; j++) {
                if(this.board[i][j].isExposed()){
                    exposedMines++;
            }
        }
            System.out.println(exposedMines);
            }
        if ((exposedMines+totalMineCount) == ((boardSize)*(boardSize))){
    ...
    private void calcAdjMine(){
        for (int i=0; i< boardSize; i++) {
            for (int j = 0; j < boardSize; j++) {
                ...
                this.board[i][j].setMineCount(count);
            }
        }
